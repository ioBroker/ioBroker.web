...
class WhiteList extends Component {
    ...
    componentDidMount() {
        this.props.socket.getUsers().then(list => {
            // try to find out IP prefix
            if (!this.ipPrefix) {
                this.props.socket
                    .getObject(`system.adapter.${this.props.adapterName}.${this.props.instance}`)
                    .then(obj => {
                        this.props.socket.getIpAddresses(obj.common.host).then(interfaces => {
                ...
                <div style={{ ...styles.column, ...styles.columnSettings, position: 'relative' }}>
                    ...
                    {!this.props.native.socketio && whiteListSettings ? (
                        <div style={native.whiteListEnabled ? null : styles.displayNone}>
                            ...
                            >
                                <Table
                                    sx={{ ...styles.table, ...styles.maxTable }}
                                            ...
                                            <TableRow key={`${index}_max`}>
                                                ...
                                                <TableCell
                                                    ...
                                                    title={
                                                        el === 'default'
                                                            ? I18n.t(
                                                                  'If no IP address matches, show authentication dialog',
                                                              )
                                                            : ''
                                                    }
                                                >
                                                ...
                                                {['object', 'state', 'file'].map((elProperty, indexProperty) =>
                                                    Object.keys(whiteListSettings[el][elProperty] || {}).map(attr => (
                                                        <TableCell
                                                            ...
                                                            style={{
                                                                borderBottom:
                                                                    indexProperty % 2 ? '1px solid #afafaf' : null,
                                                            }}
                                        ...
                                        {Object.keys(whiteListSettings).map((el, index) => (
                                            <Box
                                                ...
                                                sx={Utils.getStyle(
                                                    this.props.theme,
                                                    styles.card,
                                                    index % 2 ? styles.backgroundTheme : null,
                                                )}
                                            >
                                                ...
                                                {['object', 'state', 'file'].map((element, indexEl) => {
                                                    const newTableHeadArr = [...tableHeadArr].splice(
                                                        indexEl === 0 ? 0 : 4,
                                                        indexEl === 0 ? 4 : 5,
                                                    );
                                                    return (
                                                                    ...
                                                                    {Object.keys(
                                                                        ...
                                                                        >
                                                                            <CustomCheckbox
                                                                                table
                                                                                checked={
                                                                                    whiteListSettings[el][element][attr]
                                                                                }
                                                                                attr={attr}
                                                                                native={native}
                                                                                style={styles.checkBoxStyle}
